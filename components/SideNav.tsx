...
import {
  ...
  Megaphone, CheckSquare, BarChart2, FileText, Share2,
} from 'lucide-react'
import { getDefaultNavKeys, ROLE_LABELS, ROLE_COLORS, type AppRole } from '@/lib/rbac'

// All possible navigation items — filtered per role
const NAV_GROUPS = [
  {
    label: 'Main',
    items: [
      ...
      { key: 'campaigns', label: 'Campaigns',    icon: <Megaphone size={18}/>,       href: '/campaigns' },
      { key: 'tasks',     label: 'Tasks',         icon: <CheckSquare size={18}/>,    href: '/tasks' },
      ...
      { key: 'social',    label: 'Social Media',  icon: <Share2 size={18}/>,         href: '/social' },
    ]
  },
  {
    label: 'Analytics',
    items: [
      { key: 'reports',   label: 'Reports',      icon: <FileText size={18}/>,        href: '/reports' },
    ]
  },
  {
    label: 'Team',
    items: [
      { key: 'employees',   label: 'Employees',   icon: <Users size={18}/>,       href: '/employees' },
      { key: 'permissions', label: 'Permissions', icon: <ShieldCheck size={18}/>, href: '/permissions' },
    ]
  },
  {
    label: 'System',
...
export default function SideNav({
  ...
  // Merge role defaults with per-user overrides from Permission table
  const defaultKeys = getDefaultNavKeys(appRole)
  // Permission table entries (canAccess=true) can add extra keys for this user
  const effectiveKeys = new Set([...defaultKeys, ...permissions])

  const isVisible = (item: { key: string; always?: boolean }) => {
    if (item.always) return true
    if (effectiveKeys.has('*')) return true
    return effectiveKeys.has(item.key)
  }

  const roleColor = ROLE_COLORS[appRole] ?? '#6b7280'
  const roleLabel = ROLE_LABELS[appRole] ?? role.replace(/_/g, ' ')
    ...
    <div className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(0,0,0,0.2)' }}>
          <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: `${roleColor}22`, border: `1px solid ${roleColor}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, color: roleColor, flexShrink: 0 }}>
            {roleLabel.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2)}
          </div>
          <div className="sidebar-role-text">
            <div style={{ fontSize: '11px', color: '#e2e8f0', fontWeight: 600 }}>{roleLabel}</div>
